...
def MF(dataset,patchsize):
    model = None
    if dataset == 'Salinas':
        model = morphFormer(FM=16,NC=204,Classes=17,HSIOnly=False,patchsize=patchsize)
    elif dataset == 'PaviaU':
        model = morphFormer(FM=16,NC=103,Classes=10,HSIOnly=False,patchsize=patchsize)
    elif dataset == 'PaviaC':
        model = morphFormer(FM=16,NC=102,Classes=10,HSIOnly=False,patchsize=patchsize)
    elif dataset == 'KSC':
        model = morphFormer(FM=16,NC=176,Classes=14,HSIOnly=False,patchsize=patchsize)
    elif dataset == 'IndianPines':
        model = morphFormer(FM=16,NC=200,Classes=17,HSIOnly=False,patchsize=patchsize)
    elif dataset == 'WHUHC':
        model = morphFormer(FM=16,NC=274,Classes=17,HSIOnly=False,patchsize=patchsize)
    return model



if __name__ == '__main__':
    img = torch.randn(1, 274, 16, 16)
    print("input shape:", img.shape)
    net = MF("WHUHC",16)
    print("output shape:", net(img).shape)
    from thop import profile
    # summary(net, torch.zeros((1, 1, 200, 8, 8)))
    Flops, params = profile(net, inputs=(img,))  # macs
