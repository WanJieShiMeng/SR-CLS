...
def dffn(dataset, patch_size):
    model = None
    if dataset == 'Salinas':
        model = DFFN(bands=204, classes=17, layers_num=[3, 3, 3])

    elif dataset == 'IndianPines':
        model = DFFN(bands=200, classes=17, layers_num=[3, 3, 3])

    elif dataset == 'KSC':
        model = DFFN(bands=176, classes=14, layers_num=[3, 3, 3])
        ...
        model = DFFN(bands=274, classes=17, layers_num=[4, 4, 4])
...
if __name__ == '__main__':
    ...
    net = dffn(dataset='WHUHC', patch_size=16)
