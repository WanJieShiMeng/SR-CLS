...
def cnn3d(dataset, patch_size):
    model = None
    if dataset == 'Salinas':
        model = CNN3D(input_channels=204, n_classes=17, patch_size=patch_size)
    elif dataset == 'IndianPines':
        model = CNN3D(input_channels=200, n_classes=17, patch_size=patch_size)
    elif dataset == 'whulk':
        model = CNN3D(input_channels=270, n_classes=9, patch_size=patch_size)
    elif dataset == 'WHUHC':
        model = CNN3D(input_channels=274, n_classes=17, patch_size=patch_size)
    elif dataset == 'KSC':
...
if __name__ == '__main__':
    t = torch.randn(size=(1, 200, 16, 16))
    print("input shape:", t.shape)
    net = cnn3d(dataset='IndianPines', patch_size=16)
