...
class DSNet(nn.Module):
    ...
    """
    def __init__(self, band, num_classes, patch_size, basic_cls_name="conv2d_unmix"):
...
if __name__ == "__main__":
    x = torch.rand(1, 200, 16, 16).to(torch.device('cuda:0'))
    net = DSNet(200,17,16).to(torch.device('cuda:0'))
    y = net(x)
    print(y[2].shape)
