...
class mctgcl(nn.Module):
    ...
    def forward(self, x):
        ...
        xconv21 = rearrange(xconv21, 'b (h w) c -> b c h w', h=14, w=14)   # 11,11
...
if __name__ == '__main__':
    inputs = torch.randn(1, 30, 16, 16)
    net = mctgcl(num_classes=13, num_tokens=196)
    logits, features = mctgcl(num_classes=17, num_tokens=196)(inputs)  # 121
    print(logits.size(), features.size())

    from thop import profile

    # summary(net, torch.zeros((1, 1, 200, 8, 8)))
    Flops, params = profile(net, inputs=(inputs,))  # macs
