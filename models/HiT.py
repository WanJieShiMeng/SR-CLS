...
def hit(dataset, patchsize):
    ...
    layers = [4, 3, 14, 3]
    if dataset == 'Salinas':
        embed_dims = [408, 408, 512, 512]
        model = HiT(layers=layers, img_size=patchsize, in_chans=204, num_classes=17, embed_dims=embed_dims,
                     patch_size=3, transitions=transitions,
                     segment_dim=segment_dim, mlp_ratios=mlp_ratios, mlp_fn=ConvPermuteMLP)
    elif dataset == 'WHUHC':
        embed_dims = [552, 552, 512, 512]
        model = HiT(layers=layers, img_size=patchsize, in_chans=274, num_classes=17, embed_dims=embed_dims,
                     patch_size=3, transitions=transitions,
                     segment_dim=segment_dim, mlp_ratios=mlp_ratios, mlp_fn=ConvPermuteMLP)
    elif dataset == 'KSC':
        embed_dims = [352, 352, 512, 512]
        model = HiT(layers=layers, img_size=patchsize, in_chans=176, num_classes=14, embed_dims=embed_dims,
                     patch_size=3, transitions=transitions,
                     segment_dim=segment_dim, mlp_ratios=mlp_ratios, mlp_fn=ConvPermuteMLP)
    elif dataset == 'IndianPines':
        embed_dims = [400, 400, 512, 512]
        model = HiT(layers=layers, img_size=patchsize, in_chans=200, num_classes=17, embed_dims=embed_dims,
                     patch_size=3, transitions=transitions,
                     segment_dim=segment_dim, mlp_ratios=mlp_ratios, mlp_fn=ConvPermuteMLP)
    return model
