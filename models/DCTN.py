...
class FFN(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.2):
        ...
        self.act = act_layer()
        self.proj1 = nn.Conv2d(hidden_features, hidden_features, (3, 3), padding=(1, 1), groups=hidden_features,
                               bias=False)
...
class MDCP(nn.Module):
    ...
    def __init__(self, img_size=16, patch_size=3, in_chans=3, embed_dim=256, dataset="PaviaU"):
        ...
        self.pool = nn.AdaptiveAvgPool3d((40, img_size, img_size))
        ...
        if dataset == "PaviaU":
            ...
            groups_width = 40
            channels = 200
            self.pool = nn.AdaptiveAvgPool3d((55, img_size, img_size))
...
def dctn(dataset, patchsize):
    ...
    if dataset == 'Salinas':
        ...
        model = DCTN(layers=layers, img_size=patchsize, in_chans=204, num_classes=17, embed_dims=embed_dims,
                     patch_size=3, transitions=transitions,
                     segment_dim=segment_dim, mlp_ratios=mlp_ratios, mlp_fn=EISA, dateset=dataset)
    elif dataset == 'WHUHC':
        embed_dims = [320, 320, 512, 512]
        model = DCTN(layers=layers, img_size=patchsize, in_chans=274, num_classes=17, embed_dims=embed_dims,
                     patch_size=3, transitions=transitions,
                     segment_dim=segment_dim, mlp_ratios=mlp_ratios, mlp_fn=EISA, dateset=dataset)
    elif dataset == 'KSC':
        embed_dims = [320, 320, 512, 512]
        model = DCTN(layers=layers, img_size=patchsize, in_chans=176, num_classes=14, embed_dims=embed_dims,
                     patch_size=3, transitions=transitions,
                     segment_dim=segment_dim, mlp_ratios=mlp_ratios, mlp_fn=EISA, dateset=dataset)
    elif dataset == 'IndianPines':
        embed_dims = [440, 440, 512, 512]
        model = DCTN(layers=layers, img_size=patchsize, in_chans=200, num_classes=17, embed_dims=embed_dims,
                     patch_size=3, transitions=transitions,
                     segment_dim=segment_dim, mlp_ratios=mlp_ratios, mlp_fn=EISA, dateset=dataset)
    return model
...
if __name__ == '__main__':
    ...
    model = dctn("IndianPines", 16)#Salinas
    device = torch.device("cuda:0")
    model = model.to(device)
    model.eval()
    input = torch.randn(1,200, 16, 16).cuda()
    y = model(input)
    print(y.shape)
