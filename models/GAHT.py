...
class MyTransformer(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000, num_stages=3,
                 n_groups=[32, 32, 32], embed_dims=[256, 128, 64], num_heads=[8, 4, 2], mlp_ratios=[1, 1, 1],
                 depths=[2, 2, 2]):
        super().__init__()
...
def gaht(dataset, patch_size):
    model = None
    if dataset == 'Salinas':
        model = MyTransformer(img_size=patch_size, in_chans=204, num_classes=17, n_groups=[16, 16, 16],depths=[2, 1, 1])
    elif dataset == 'PaviaU':
        model = MyTransformer(img_size=patch_size, in_chans=103, num_classes=10, n_groups=[2, 2, 2], depths=[1, 2, 1])
    elif dataset == 'PaviaC':
        model = MyTransformer(img_size=patch_size, in_chans=102, num_classes=10, n_groups=[2, 2, 2], depths=[1, 2, 1])
    elif dataset == 'whulk':
        model = MyTransformer(img_size=patch_size, in_chans=270, num_classes=9, n_groups=[2, 2, 2], depths=[2, 2, 1])
    elif dataset == 'WHUHC':
        model = MyTransformer(img_size=patch_size, in_chans=274, num_classes=17, n_groups=[2, 2, 2], depths=[2, 2, 1])
    elif dataset == 'hrl':
        ...
        model = MyTransformer(img_size=patch_size, in_chans=200, num_classes=17, n_groups=[4, 4, 4], depths=[1, 2, 1])
    elif dataset == 'hus':
...
if __name__ == "__main__":
    t = torch.randn(size=(1, 200, 8, 8))
    print("input shape:", t.shape)
    net = gaht(dataset='IndianPines', patch_size=8)
