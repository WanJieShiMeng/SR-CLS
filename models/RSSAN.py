...
class RSSAN(nn.Module):
    def __init__(self, n_bands, kernel_number, patch_size, n_classes):
        ...
        self.block1 = nn.Sequential(
            ...
            nn.BatchNorm2d(num_features=kernel_number, eps=0.001, momentum=0.1, affine=True),
...
def rssan(dataset, patch_size):
    model = None
    if dataset == 'Salinas':
        model = RSSAN(n_bands=204, kernel_number=32, patch_size=patch_size, n_classes=17)
    elif dataset == 'IndianPines':
        model = RSSAN(n_bands=200, kernel_number=32, patch_size=patch_size, n_classes=17)
    elif dataset == 'pu':
        model = RSSAN(n_bands=103, kernel_number=8, patch_size=patch_size, n_classes=9)
    elif dataset == 'whulk':
        model = RSSAN(n_bands=270, kernel_number=32, patch_size=patch_size, n_classes=9)
    elif dataset == 'WHUHC':
        model = RSSAN(n_bands=274, kernel_number=32, patch_size=patch_size, n_classes=17)
    elif dataset == 'KSC':
...
if __name__ == '__main__':
    t = torch.randn(size=(1, 274, 16, 16))
    print("input shape:", t.shape)
    net = rssan(dataset='WHUHC', patch_size=16)
    print("output shape:", net(t).shape)

    from thop import profile
    Flops, params = profile(net, inputs=(t,))  # macs
