...
try:
    from dataset.custom_datasets import CUSTOM_DATASETS_CONFIG
...
def get_dataset(dataset_name, target_folder="./", datasets=DATASETS_CONFIG, use_PCA=True):
    ...
    if use_PCA:
        ...
        set_random_seed(202401) # 保证每次pca结果一样
...
class HyperX_multi(torch.utils.data.Dataset):
    """ 在getitem的时候进行数据处理,时间太慢 """

    def __init__(self, data, gt, is_multi_task = True, **hyperparams):
        """
        Args:
            data: 3D hyperspectral image
            gt: 2D array of labels
            ...
            supervision: 'full' or 'semi' supervised algorithms
        """
        super(HyperX_multi, self).__init__()
    ...
    def __getitem__(self, i):
        ...
        if self.is_multi_task:
            lr_data = degradation(data)
            lr_data = np.asarray(np.copy(lr_data).transpose((2, 0, 1)), dtype="float32")
        ...
        #----------------------------------------------------------------------------------#
        # bic_data = imresize(lr_data,2)
        if self.is_multi_task:
            return data, label, lr_data
        else:
            return data, label


class HyperX_2(torch.utils.data.Dataset):
    """ Generic class for a hyperspectral scene """

    def __init__(self, oir_data, gt, **hyperparams):
        """
        Args:
            data: 3D hyperspectral image
            gt: 2D array of labels
        ...
        self.indices = np.array(
            [
                (x, y)
                for x, y in zip(x_pos, y_pos)
                if x >= p and x <= oir_data.shape[0] - p and y >= p and y <= oir_data.shape[1] - p
            ]
        )
        self.labels = [self.label[x, y] for x, y in self.indices]
        ...
        for i in range(self.indices.shape[0]):
            x, y = self.indices[i]
            data = self.data[(x - self.patch_size // 2):(x + self.patch_size // 2),
    ...
    def __getitem__(self, i):
        ...
        return self.input_data[i], self.input_label[i]





class HyperX_multi_2(torch.utils.data.Dataset):
    """ 先对数据进行处理,然后选择,这样泛化能力较差 """
    ...
    def __init__(self, data, gt, is_multi_task = True, **hyperparams):
        ...
        Args:
            ...
            supervision: 'full' or 'semi' supervised algorithms
        """
        super(HyperX_multi_2, self).__init__()
        self.is_multi_task = is_multi_task
        self.data = data
        self.label = gt
        self.name = hyperparams["dataset"]
        ...
        for i in range(self.indices.shape[0]):
            ...
            self.input_data.append(data)
            self.input_label.append(label)
            self.input_lr.append(lr_data)
            self.input_bic_data.append(bic_data)
    ...
    def __getitem__(self, i):
        if self.is_multi_task:
            return self.input_data[i], self.input_label[i], self.input_lr[i], self.input_bic_data[i]
        else:
            return self.input_lr[i], self.input_label[i]


def degradation(x,sf=2):
    # x的size大小为16
    k = anisotropic_Gaussian(ksize=8, theta=np.pi, l1=9, l2=9)
    x = srmd_degradation(x,k,sf)
    x = noise(x,10)
    return x
...
def noise(x,noise_level):
    x += np.random.normal(0, noise_level / 255.0, x.shape).astype(np.float32)
    x = np.clip(x, 0.0, 1.0)
    return x
...
if __name__ == "__main__":
    ...
    imshow(k * 10)
    img_s = srmd_degradation(img, k, sf=4)